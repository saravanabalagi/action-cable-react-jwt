...
that a successful authentication got you the JWT token. This JWT has to be sent when a new connection is attempted, and you
...
@App = {}
var jwt = getToken();     //your token here
App.cable = ActionCable.createConsumer("ws://cable.example.com", jwt)
```
...
In addition to being available through the `actioncable-jwt` gem, Action Cable's
...
```
npm install actioncable-jwt --save
```
...
var cable = ActionCable.createConsumer('wss://RAILS-API-PATH.com/cable', jwt)
...
```coffeescript
ActionCable = require('actioncable')
jwt = getToken()  # your token here

cable = ActionCable.createConsumer('wss://RAILS-API-PATH.com/cable', jwt)
